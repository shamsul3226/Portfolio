...
class PortfolioApp {
    ...
    setupScrollEffects() {
        ...
        document.querySelectorAll('.glass-card, .skill-category, .project-card').forEach(el => {
            observer.observe(el);
        });
    }
    ...
    showFormSuccess() {
        ...
        const notification = this.createNotification('Message sent successfully! I\'ll get back to you soon.', 'success');
    ...
    setupTypingEffect() {
        const nameElement = document.querySelector('.bg-gradient-to-r.from-neon-cyan');
        if (nameElement) {
    ...
    setupResumeDownload() {
        ...
        if (resumeBtn) {
            resumeBtn.addEventListener('click', (e) => {
                ...
                const notification = this.createNotification('Resume download started!', 'success');
                document.body.appendChild(notification);
